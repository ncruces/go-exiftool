    ...
    if ( defined *$self->{FH} || defined *$self->{InputEvent} ) {
        ...
        *$self->{InputLengthRemaining} += length( $_[0] );
        ...
        if ( $len > *$self->{BufferOffset} ) {
            *$self->{Prime} = substr( $_[0], 0, $len - *$self->{BufferOffset} )
              . *$self->{Prime};
...
sub checkParams {
    ...
    my $Valid = {
        'blocksize' =>
          [ IO::Compress::Base::Common::Parse_unsigned, 16 * 1024 ],
    ...
    $Valid->{trailingdata} =
      [ IO::Compress::Base::Common::Parse_writable_scalar, undef ]
      if *$self->{OneShot};
...
sub _inf {
    ...
    my $x =
      IO::Compress::Base::Validator->new( $class, *$obj->{Error}, $name,
        $input, $output )
      or return undef;
...
sub _rd2 {
    ...
    my $z = IO::Compress::Base::Common::createSelfTiedObject( $x->{Class},
        *$self->{Error} );
...
sub readBlock {
    ...
    if ( defined *$self->{CompressedInputLength} ) {
        ...
        $size =
          List::Util::min( $size, *$self->{CompressedInputLengthRemaining} );
...
sub seek {
    ...
    while (
        (
            $got = $self->read(
                my $buffer, List::Util::min( $offset, *$self->{BlockSize} )
            )
        ) > 0
      )
    {
