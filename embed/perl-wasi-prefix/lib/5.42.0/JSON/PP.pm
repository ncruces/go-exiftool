...
sub boolean_values {
    ...
    if (@_) {
        ...
        if (CORE_BOOL) {
            ...
            BEGIN {
                CORE_BOOL and warnings->unimport(qw(experimental::builtin));
            }
            if (   builtin::is_bool($true)
                && builtin::is_bool($false)
                && $true
                && !$false )
            {
                $self->{core_bools} = !!1;
...
"malformed JSON string, neither array, object, number, string or atom"
        ...
        (
            $utf8,        $relaxed, $loose, $allow_bignum, $allow_barekey,
            $singlequote, $allow_tags
          )
                    ...
                    if ( !$loose ) {
                        if ( $ch =~ $invalid_char_re ) {
                            if ( !$relaxed or $ch ne "\t" ) {
                                $at--;
                                decode_error(
                                    sprintf "invalid character 0x%X"
...
'malformed JSON string, neither array, object, number, string or atom'
        ...
        decode_error('malformed JSON string, (tag) must be a string')
          if ref $tag;
        ...
        if ( !eval { $tag->can('THAW') } ) {
            decode_error('cannot decode perl-object (package does not exist)')
              if $@;
            decode_error(
...
"attempted decode of JSON text of $self->{incr_pos} bytes size, but max_size is set to $max_size"
                ...
                my ( $obj, $offset ) =
                  $coder->PP_decode_json( $self->{incr_text}, 0x00000001 );
                push @ret, $obj;
                use bytes;
    ...
    while ( $len > $p ) {
        ...
        if ( $mode == INCR_M_WS ) {
            ...
            while ( $len > $p ) {
                ...
                if ( $s eq "\n" ) {
                    $self->{incr_mode} =
                      $self->{incr_mode} == INCR_M_C0 ? INCR_M_WS : INCR_M_JSON;
                    last;
                }
                $p++;
            }
