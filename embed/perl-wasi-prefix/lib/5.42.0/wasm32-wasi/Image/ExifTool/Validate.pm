...
my %validValue = (
    JPEG => {
        ...
        ExifIFD => {
            0x9000 => 'defined $val and $val =~ /^\d{4}$/',
            0x9101 => 'defined $val',

            0xa001 => '$val == 1 or $val == 0xffff',
            0xa002 => 'defined $val',
            0xa003 => 'defined $val',
        },
        GPS => {
            0x00 => 'defined $val and $val =~ /^\d \d \d \d$/',
            0x1b => 'not defined $val or $val =~ /^(GPS|CELLID|WLAN|MANUAL)$/',
        },
        InteropIFD => {},
    },
    TIFF => {
        IFD0 => {
            ...
            0x11a => 'defined $val',
            0x11b => 'defined $val',
            0x128 => 'not defined $val or $val =~ /^[123]$/',
        ...
        GPS => {
            0x00 => 'defined $val and $val =~ /^\d \d \d \d$/',
            0x1b => '$val =~ /^(GPS|CELLID|WLAN|MANUAL)$/',
        },
    },
);
...
sub ValidateRaw($$$) {
    ...
    if ( $$tagInfo{Validate} ) {
        ...
        if ( $wrn or $err ) {
            my $name = $$tagInfo{Table}{GROUPS}{0} . ':'
