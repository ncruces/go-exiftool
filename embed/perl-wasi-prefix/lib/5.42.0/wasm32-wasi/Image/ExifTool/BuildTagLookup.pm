...
"Warning: Special characters in $short $name PrintConv ($$printConv{$_})\n";
                ...
                if ( $subdir and not $$tagInfo{SeparateTable} ) {
                    if ($writable) {
                        ...
                        $writable = 'yes'
                          if $tw and $writable eq '1' or $writable eq '2';
                        $writable = '-' . ( $tw ? $writable : '' );
                        $writable .= '!'
                          if $tw and ( $$tagInfo{Protected} || 0 ) & 0x01;
                        $writable .= '+'
                          if $$tagInfo{List} or $$tagInfo{IsList};
                    ...
                    if ( not $writable ) {
                        ...
                        $writable = $format ? $format : 'yes'
                          if $writable eq '1' or $writable eq '2';
...
sub GetTableOrder(@) {
    ...
    foreach $pos ( sort { $b <=> $a } keys %fixPos ) {
        my $fix = $fixPos{$pos};
        foreach (@$fix) {
